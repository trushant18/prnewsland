...
@section('content')
    <div class="slim-pageheader">
        ...
        <h6 class="slim-pagetitle">Blog List</h6>
    </div>
            ...
            <table id="dataTable" class="table display responsive nowrap">
                ...
                @foreach($blog_list as $item)
                    <tr>
                        <td>{{ $item->title }}</td>
                        <td>
                            <a href="{{ route('admin.blog.edit', $item->id) }}" class="btn btn-info btn-sm" title="Edit">
                                <div><i class="fa fa-edit"></i></div>
                            </a>
                            <a href="javascript:;" onclick="confirmDelete('{{ route("admin.blog.delete",$item->id) }}')" class="btn btn-danger btn-sm" title="Delete">
